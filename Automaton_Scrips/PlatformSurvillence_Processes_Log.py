...
def PlatformSurvillance(FolderName):
    ...
    if(Ret == True):
        ...
        os.mkdir(FolderName) # Make a Folder

        print("Directory for the logfile gets created succesfully")

    timestamp = time.strftime("%Y-%M-%d_%H-%M-%S")

    FileName = os.path.join(FolderName,"Marvellouss_%s.log" %timestamp) # File Direct Tychy AAt Padal(MarvellousLog )
...
# Netork Usage
    ...
    fobj.write("Sent : %.2f MB\n" %(netobj.bytes_sent / (1024 * 1024))) # (KB * MB)
...
def main():
    ...
    # Actual project code
        ...
        print("Shedular Started Successfully")
